...
public class PVPCraftMenu : MonoBehaviour {
    ...
    public void Init(string con, PlayerControl p) {
        ...
        if (con == "keyboard")
        {
            ...
            whichPlayer = con;
            useController = true;
            Debug.Log(playerControl.gameObject.name);
        }
    }
    ...
    void GetInput()
    {
        if (useController)
        {
            if (!onMoving && Input.GetButtonDown(whichPlayer + "ButtonY"))
            {
                ...
                showUp = !showUp;
                onMoving = true;
                if (showUp) playerControl.SetMenuMode(true);
                else playerControl.SetMenuMode(false);
            }
        }
        else
        {
